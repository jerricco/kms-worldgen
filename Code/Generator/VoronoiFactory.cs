...
public sealed class VoronoiFactory : Component
{
    ...
    public float GetContinuousElevationAt(Vector2 point)
    {
	    ...
	    Vector2 tPos = new Vector2(point.x + MapManager.OffsetX, point.y + MapManager.OffsetY);
	    ...
	    Vector2 sPos = new Vector2(rPos.x * noiseScale, rPos.y * noiseScale);
	    
	    float noiseX = (float)MapManager.Noise.Evaluate(sPos.x, sPos.y) * noiseStrength;
	    float noiseY = (float)MapManager.Noise.Evaluate(sPos.y, sPos.x) * noiseStrength;
	    ...
	    {
		    float distToThisSpine = DistanceToSpine(warpedPoint, spine);
		    if (blendedDistance == float.MaxValue) blendedDistance = distToThisSpine;
		    else blendedDistance = SmoothMin(blendedDistance, distToThisSpine, smoothBlendRadius);
	    }
