...
public sealed class ProceduralDelaunayRenderer : Component
{
    ...
    public void RebuildMesh(Delaunator delaunayData)
    {
        ...
        this.LineMaterial = this.LineMaterial ?? Material.FromShader("materials/opaque_line.vmat");
        ...
        if (!this._modelRenderer.Enabled)
        ...
        const float startHue = 120f;
        const float endHue = 0f;
        ...
        void TryAddLineIndexPair(int aIdx, int bIdx)
        {
            ...
            if (!processedEdges.Add(edgeKey)) return;
