using System;
using static Sandbox.Triangulation.Delaunator;

namespace Sandbox.Triangulation;
// Utilising https://github.com/BorisTheBrave/voronator-sharp
// Which also utilises https://github.com/nol1fe/delaunator-sharp
// Credit to library author(s).

/// <summary>
/// Computes a voronoi diagram in a 2d plane,
/// optionally clipped to a rectangle.
/// </summary>
public class Voronator
{
	public enum PolygonStatus
	{
		/// <summary>
		/// A normal polygon
		/// </summary>
		Normal,

		/// <summary>
		/// A polygon on the boundary, it extends outwards indefinitely.
		/// </summary>
		Infinite,

		/// <summary>
		/// Single point occupies all of space
		/// </summary>
		Solo,

		/// <summary>
		/// All the points are in a line, and this is not at either end of the line.
		/// </summary>
		Collinear,

		/// <summary>
		/// Something else has gone wrong, e.g. duplicate point.
		/// </summary>
		Error,
	}

	private readonly Vector2 clipMax;

	// Clipping rectangle used for GetClippedPolygon.
	private readonly Vector2 clipMin;

	// triangle circumcenters, plus some extra vertices on the end

	// In collinear situations, gives the normal to the line the cells run in.
	// In this case, we don't use a delaunay triangulation at all.
	// Instead we use Delaunator.Hull which contains an ordered set of vertices.
	private Vector2? collinearNormal;

	// Delaunay triangulation dual to the voronoi cells

	// Inverse lookup of d.Hull
	private int[] hullIndex;

	private IList<Vector2> points;

	// Vectors holds two vectors for each hull point, indicating the directions of the rays outwards
	private Vector2[] vectors;

	public Voronator(IList<Vector2> points, Vector2 clipMin, Vector2 clipMax)
	{
		this.Init(points);
		this.clipMin = clipMin;
		this.clipMax = clipMax;
	}

	public Voronator(IList<Vector2> points)
	{
		this.Init(points);

		// Set bounds.
		this.clipMin = this.clipMax = points[0];
		for (var i = 1; i < points.Count; i++)
		{
			this.clipMin = Vector2.Min(this.clipMin, points[i]);
			this.clipMax = Vector2.Max(this.clipMax, points[i]);
		}

		for (var i = 0; i < this.TriangleVertices.Count; i++)
		{
			this.clipMin = Vector2.Min(this.clipMin, this.TriangleVertices[i]);
			this.clipMax = Vector2.Max(this.clipMax, this.TriangleVertices[i]);
		}

		// Inflate the clipping area slightly
		// This ensures that unbounded cells will still share a border
		// after clipping.
		var expand = 1e-6f * (this.clipMax - this.clipMin);
		this.clipMin -= expand;
		this.clipMax += expand;
	}

	// Delaunay triangulation dual to the voronoi cells
	public Delaunator Delaunator { get; private set; }

	// triangle circumcenters
	public List<Vector2> TriangleVertices { get; private set; }

	// Map from point index to half edge.
	// The starting half edge is the one with d.Halfedges[e] == -1
	// Or an arbitrary choice if there isn't one.
	/// <summary>
	/// Map from point index to a half edge.
	/// The starting half edge is the one with d.Halfedges[e] == -1.
	/// Or an arbitrary choice if there isn't one.
	/// </summary>
	public int[] Inedges { get; private set; }

	public Vector2 ClipMin => this.clipMin;
	public Vector2 ClipMax => this.clipMax;

	private void Init(IList<Vector2> points)
	{
		this.points = points;

		// Actually do Delaunay triangulation
		this.Delaunator = new Delaunator(points.ToArray());

		if (this.Delaunator.Hull.Length > 0 && this.Delaunator.Triangles.Length == 0)
		{
			// The points are collinear
			var first = points[this.Delaunator.Hull[0]];
			var last = points[this.Delaunator.Hull[this.Delaunator.Hull.Length - 1]];
			var l = last - first;

			var cPos = new Vector2(l.y, -l.x);
			var mag = (float)Math.Sqrt((cPos.x * cPos.x) + (cPos.y * cPos.y));
			var normalised = mag > 0.00001f ? new Vector2(cPos.x / mag, cPos.y / mag) : new Vector2(0, 0);
			this.collinearNormal = normalised;
		}

		// Inverse lookup of hull
		this.hullIndex = new int[points.Count];
		for (var i = 0; i < this.hullIndex.Length; ++i)
		{
			this.hullIndex[i] = -1;
		}

		for (var i = 0; i < this.Delaunator.Hull.Length; ++i)
		{
			this.hullIndex[this.Delaunator.Hull[i]] = i;
		}

		// Load the vertices of the mesh
		this.TriangleVertices = new List<Vector2>();
		var triangleCount = this.Delaunator.Triangles.Length / 3;
		for (var triangleId = 0; triangleId < triangleCount; triangleId++)
		{
			this.TriangleVertices.Add(this.Delaunator.GetTriangleCircumcenter(triangleId));
		}

		// Find the voronoi cells, storing the "starting" half edge.
		this.Inedges = new int[points.Count];
		for (var e = 0; e < points.Count; ++e)
		{
			this.Inedges[e] = -1;
		}

		for (var e = 0; e < this.Delaunator.Halfedges.Length; ++e)
		{
			var p = this.Delaunator.Triangles[NextHalfedge(e)];
			if (this.Delaunator.Halfedges[e] == -1 || this.Inedges[p] == -1)
			{
				this.Inedges[p] = e;
			}
		}

		// Compute exterior rays
		this.vectors = new Vector2[this.Delaunator.Points.Count * 2];
		var h = this.Delaunator.Hull[this.Delaunator.Hull.Length - 1];
		int p0, p1 = h * 2;
		Vector2 v0, v1 = points[h];
		for (var i = 0; i < this.Delaunator.Hull.Length; i++)
		{
			h = this.Delaunator.Hull[i];
			(p0, v0) = (p1, v1);
			p1 = h * 2;
			v1 = points[h];
			this.vectors[p0 + 1] = this.vectors[p1] = new Vector2(v0.y - v1.y, v1.x - v0.x);
		}
	}

	public PolygonStatus GetPolygonStatus(int i)
	{
		if (i == 0 && this.Delaunator.Hull.Length == 1)
		{
			return PolygonStatus.Solo;
		}

		if (this.collinearNormal != null)
		{
			// Corresponds to ClipCollinear

			var n = this.collinearNormal.Value;
			var hi = this.hullIndex[i];
			if (hi == 0)
			{
				return PolygonStatus.Infinite;
			}

			if (hi == this.Delaunator.Hull.Length - 1)
			{
				return PolygonStatus.Infinite;
			}

			if (hi == -1)
			{
				return PolygonStatus.Error;
			}

			return PolygonStatus.Collinear;
		}

		// Corresponds to null == GetPolygon(i)
		var e0 = this.Inedges[i];
		if (e0 == -1)
		{
			return PolygonStatus.Error;
		}

		var v = i * 2;
		if (this.vectors[v] == default)
		{
			// Corresponds to ClipFinite
			return PolygonStatus.Normal;
		}

		// Corresponds to ClipInfinite
		return PolygonStatus.Infinite;
	}

	/// <summary>
	/// Returns the centroid of each voronoi cell.
	/// This is suitable for use with Lloyd relaxation.
	/// Unbounded cells return their original point.
	/// </summary>
	public List<Vector2> GetRelaxedPoints()
	{
		var relaxedPoints = new List<Vector2>();
		for (var i = 0; i < this.points.Count; i++)
		{
			var polygonStatus = this.GetPolygonStatus(i);
			var relaxedPoint = polygonStatus == PolygonStatus.Normal ? GetCentroid(this.GetPolygon(i)) : this.points[i];
			relaxedPoints.Add(relaxedPoint);
		}

		return relaxedPoints;
	}

	/// <summary>
	/// Returns the centroid of each voronoi cell.
	/// This is suitable for use with Lloyd relaxation.
	/// Unbounded cells are clipped down, which tends to move them inwards.
	/// </summary>
	public List<Vector2> GetClippedRelaxedPoints()
	{
		var relaxedPoints = new List<Vector2>();
		for (var i = 0; i < this.points.Count; i++)
		{
			var polygon = this.GetClippedPolygon(i);
			var relaxedPoint = polygon == null ? this.points[i] : GetCentroid(polygon);
			relaxedPoints.Add(relaxedPoint);
		}

		return relaxedPoints;
	}

	public static Vector2 GetCentroid(List<Vector2> points)
	{
		var accumulatedArea = 0.0f;
		var centerX = 0.0f;
		var centerY = 0.0f;

		for (int i = 0, j = points.Count - 1; i < points.Count; j = i++)
		{
			var temp = (points[i].x * points[j].y) - (points[j].x * points[i].y);
			accumulatedArea += temp;
			centerX += (points[i].x + points[j].x) * temp;
			centerY += (points[i].y + points[j].y) * temp;
		}

		if (Math.Abs(accumulatedArea) < 1E-7f)
		{
			return new Vector2();
		}

		accumulatedArea *= 3f;
		return new Vector2(centerX / accumulatedArea, centerY / accumulatedArea);
	}

	private IEnumerable<(Vector2, Vector2)> GetVoronoiEdges()
	{
		/*
		for (var e = 0; e < d.Triangles.Length; e++)
		{
		    var o = d.Halfedges[e];
		    if (e < o)
		    {
		        var p = triangleVertices[D.TriangleOfEdge(e)];
		        var q = triangleVertices[D.TriangleOfEdge(o)];
		        yield return new Edge(e, p, q);
		    }
		}
		*/
		// TODO: Think about this
		return null;
	}

	#region d3-delaunay

	/// <summary>
	/// Returns the vertices of the voronoi cell, without any clipping.
	/// This means that unbounded cells will be missing the edges that extend to infinity,
	/// and may have less than 3 vertices.
	/// </summary>
	public List<Vector2> GetPolygon(int i)
	{
		var vertices = new List<Vector2>();
		if (this.GetPolygon(i, vertices, out _, out _))
		{
			return vertices;
		}

		return null;
	}

	/// <summary>
	/// Supplies the vertices of the voronoi cell, without any clipping.
	/// This means that unbounded cells will be missing the edges that extend to infinity,
	/// and may have less than 3 vertices.
	/// </summary>
	/// <param name="i">The voronoi cell</param>
	/// <param name="vertices">Filled with the vertices of the polygon.</param>
	/// <param name="ray1">For unbounded cells, the direction of the ray extending from vertex 0. Otherwise, empty.</param>
	/// <param name="ray2">For unbounded cells, the direction of the ray extending from vertex 0. Otherwise, empty.</param>
	/// <returns>True if successful</returns>
	public bool GetPolygon(int i, List<Vector2> vertices, out Vector2 ray1, out Vector2 ray2)
	{
		if (this.collinearNormal != null)
		{
			// There's nothing sensible to return here except for the ends
			// of the hull, it's not worth worrying about
			ray1 = ray2 = default;
			return false;
		}

		var e0 = this.Inedges[i];
		if (e0 == -1)
		{
			ray1 = ray2 = default;
			return false;
		}

		vertices.Clear();
		var e = e0;
		do
		{
			vertices.Add(this.TriangleVertices[EdgeIndexToTriangleIndex(e)]);
			e = this.Delaunator.Halfedges[NextHalfedge(e)];
		}
		while (e != e0 && e != -1);

		ray1 = this.vectors[i * 2];
		ray2 = this.vectors[(i * 2) + 1];
		return true;
	}

	/// <summary>
	/// Returns the vertices of the voronoi cell i after clipping to the clipping rectangle.
	/// Returns null if the polygon is fully outside the clipping rectangle.
	/// </summary>
	public List<Vector2> GetClippedPolygon(int i)
	{
		// degenerate case (1 valid point: return the box)
		if (i == 0 && this.Delaunator.Hull.Length == 1)
		{
			return new List<Vector2>
			{
				new(this.clipMax.x, this.clipMin.y), this.clipMax, new(this.clipMin.x, this.clipMax.y), this.clipMin,
			};
		}

		if (this.collinearNormal != null)
		{
			return this.ClipCollinear(i);
		}

		var points = this.GetPolygon(i);
		var v = i * 2;
		if (this.vectors[v] == default)
		{
			return this.ClipFinite(i, points);
		}

		return this.ClipInfinite(i, points, this.vectors[v], this.vectors[v + 1]);
	}

	/// Clips voronoi cell i, a polygon with the given points to the clipping rectangle.
	/// Returns null if the polygon doesn't intersect the bounds.
	private List<Vector2> ClipFinite(int i, List<Vector2> points)
	{
		if (points == null)
		{
			return null;
		}

		var n = points.Count;
		List<Vector2> P = null;
		Vector2 v0;
		var v1 = points[n - 1];
		int c0;
		var c1 = this.RegionCode(v1);
		int e0, e1 = 0;
		for (var j = 0; j < n; j += 1)
		{
			v0 = v1;
			v1 = points[j];
			c0 = c1;
			c1 = this.RegionCode(v1);
			if (c0 == 0 && c1 == 0)
			{
				e0 = e1;
				e1 = 0;
				if (P != null)
				{
					P.Add(v1);
				}
				else
				{
					P = new List<Vector2>
					{
						v1,
					};
				}
			}
			else
			{
				(Vector2, Vector2)? S;
				Vector2 s0, s1;
				if (c0 == 0)
				{
					if ((S = this.ClipSegment(v0, v1, c0, c1)) == null)
					{
						continue;
					}

					(s0, s1) = S.Value;
				}
				else
				{
					if ((S = this.ClipSegment(v1, v0, c1, c0)) == null)
					{
						continue;
					}

					(s1, s0) = S.Value;
					e0 = e1;
					e1 = this.EdgeCode(s0);
					if (e0 > 0 && e1 > 0)
					{
						this.Edge(i, e0, e1, P, P.Count);
					}

					if (P != null)
					{
						P.Add(s0);
					}
					else
					{
						P = new List<Vector2>
						{
							s0,
						};
					}
				}

				e0 = e1;
				e1 = this.EdgeCode(s1);
				if (e0 > 0 && e1 > 0)
				{
					this.Edge(i, e0, e1, P, P.Count);
				}

				if (P != null)
				{
					P.Add(s1);
				}
				else
				{
					P = new List<Vector2>
					{
						s1,
					};
				}
			}
		}

		if (P != null)
		{
			e0 = e1;
			e1 = this.EdgeCode(P[0]);
			if (e0 > 0 && e1 > 0)
			{
				this.Edge(i, e0, e1, P, P.Count);
			}
		}
		else if (this.Contains(i, (this.clipMin + this.clipMax) / 2))
		{
			return new List<Vector2>
			{
				new(this.clipMax.x, this.clipMin.y), this.clipMax, new(this.clipMin.x, this.clipMax.y), this.clipMin,
			};
		}

		return P;
	}

	// Clips a segment against the bounding rect.
	// You must have already computed the regioncodes for each vertex
	private (Vector2, Vector2)? ClipSegment(Vector2 v0, Vector2 v1, int c0, int c1)
	{
		while (true)
		{
			if (c0 == 0 && c1 == 0)
			{
				return (v0, v1);
			}

			if ((c0 & c1) != 0)
			{
				return null;
			}

			var c = c0 != 0 ? c0 : c1;
			Vector2 v;
			if ((c & 0b1000) != 0)
			{
				v = new Vector2(v0.x + ((v1.x - v0.x) * (this.clipMax.y - v0.y) / (v1.y - v0.y)), this.clipMax.y);
			}
			else if ((c & 0b0100) != 0)
			{
				v = new Vector2(v0.x + ((v1.x - v0.x) * (this.clipMin.y - v0.y) / (v1.y - v0.y)), this.clipMin.y);
			}
			else if ((c & 0b0010) != 0)
			{
				v = new Vector2(this.clipMax.x, v0.y + ((v1.y - v0.y) * (this.clipMax.x - v0.x) / (v1.x - v0.x)));
			}
			else
			{
				v = new Vector2(this.clipMin.x, v0.y + ((v1.y - v0.y) * (this.clipMin.x - v0.x) / (v1.x - v0.x)));
			}

			if (c0 != 0)
			{
				v0 = v;
				c0 = this.RegionCode(v0);
			}
			else
			{
				v1 = v;
				c1 = this.RegionCode(v1);
			}
		}
	}

	/// Clips voronoi cell i, an open polygon with the given points and two open rays, to the clipping rectangle.
	/// Returns null if the polygon doesn't intersect the bounds.
	private List<Vector2> ClipInfinite(int i, List<Vector2> points, Vector2 v0, Vector2 vn)
	{
		if (points == null)
		{
			return null;
		}

		var P = new List<Vector2>(points);
		Vector2? p;
		if ((p = this.Project(P[0], v0)).HasValue)
		{
			P.Insert(0, p.Value);
		}

		if ((p = this.Project(P[P.Count - 1], vn)).HasValue)
		{
			P.Add(p.Value);
		}

		if ((P = this.ClipFinite(i, P)) != null)
		{
			var n = P.Count;
			int c0, c1 = this.EdgeCode(P[n - 1]);
			for (var j = 0; j < n; j += 1)
			{
				c0 = c1;
				c1 = this.EdgeCode(P[j]);
				if (c0 != 0 && c1 != 0)
				{
					j = this.Edge(i, c0, c1, P, j);
					n = P.Count;
				}
			}
		}
		else if (this.Contains(i, (this.clipMin + this.clipMax) / 2))
		{
			return new List<Vector2>
			{
				this.clipMin, new(this.clipMax.x, this.clipMin.y), this.clipMax, new(this.clipMin.x, this.clipMax.y),
			};
		}

		return P;
	}

	/// <summary>
	/// Clips the voronoi cell to the clipping rectangle.
	/// This handles the case that there's only collinear points,
	/// which means that the polygon can be open on both sides.
	/// </summary>
	private List<Vector2> ClipCollinear(int i)
	{
		var n = this.collinearNormal.Value;
		var hi = this.hullIndex[i];
		if (hi == 0)
		{
			var v0 = this.points[this.Delaunator.Hull[0]];
			var v1 = this.points[this.Delaunator.Hull[1]];
			var points = new List<Vector2>
			{
				(v0 + v1) / 2,
			};
			return this.ClipInfinite(i, points, n, -n);
		}

		if (hi == this.Delaunator.Hull.Length - 1)
		{
			var v0 = this.points[this.Delaunator.Hull[hi - 1]];
			var v1 = this.points[this.Delaunator.Hull[hi]];
			var points = new List<Vector2>
			{
				(v0 + v1) / 2,
			};
			return this.ClipInfinite(i, points, -n, n);
		}

		if (hi == -1)
		{
			return null;
		}

		{
			var v0 = this.points[this.Delaunator.Hull[hi - 1]];
			var v1 = this.points[this.Delaunator.Hull[hi]];
			var v2 = this.points[this.Delaunator.Hull[hi + 1]];
			var m1 = (v0 + v1) / 2;
			var m2 = (v1 + v2) / 2;
			var P = new List<Vector2>();
			Vector2? p;
			if ((p = this.Project(m1, -n)).HasValue)
			{
				P.Add(p.Value);
			}

			if ((p = this.Project(m1, n)).HasValue)
			{
				P.Add(p.Value);
			}

			if ((p = this.Project(m2, n)).HasValue)
			{
				P.Add(p.Value);
			}

			this.Edge(i, this.EdgeCode(P[P.Count - 2]), this.EdgeCode(P[P.Count - 1]), P, P.Count - 1);

			if ((p = this.Project(m2, -n)).HasValue)
			{
				P.Add(p.Value);
			}

			this.Edge(i, this.EdgeCode(P[P.Count - 1]), this.EdgeCode(P[0]), P, P.Count);
			return P;
		}
	}

	// Given an clipped segment of cell i that goes from edge code e0 to e1,
	// finds the appropriate corner points to insert into P at j.
	// Also removes co-linear edges.
	private int Edge(int i, int e0, int e1, List<Vector2> P, int j)
	{
		// Walk round any corners needed
		while (e0 != e1)
		{
			Vector2 u;
			switch (e0)
			{
				case 0b0101:
					e0 = 0b0100;
					continue;// top-left
				case 0b0100:
					e0 = 0b0110;
					u = new Vector2(this.clipMax.x, this.clipMin.y);
					break;// top
				case 0b0110:
					e0 = 0b0010;
					continue;// top-right
				case 0b0010:
					e0 = 0b1010;
					u = new Vector2(this.clipMax.x, this.clipMax.y);
					break;// right
				case 0b1010:
					e0 = 0b1000;
					continue;// bottom-right
				case 0b1000:
					e0 = 0b1001;
					u = new Vector2(this.clipMin.x, this.clipMax.y);
					break;// bottom
				case 0b1001:
					e0 = 0b0001;
					continue;// bottom-left
				case 0b0001:
					e0 = 0b0101;
					u = new Vector2(this.clipMin.x, this.clipMin.y);
					break;// left
				default: throw new Exception();
			}

			if ((j < 0 || j >= P.Count || P[j] != u) && this.Contains(i, u))
			{
				P.Insert(j, u);
				j++;
			}
		}

		// Look for collinear points to remove
		if (P.Count > 2)
		{
			for (var i2 = 0; i2 < P.Count; i2 += 1)
			{
				var j2 = (i2 + 1) % P.Count;
				var k = (i2 + 2) % P.Count;
				if ((P[i2].x == P[j2].x && P[j2].x == P[k].x) || (P[i2].y == P[j2].y && P[j2].y == P[k].y))
				{
					P.RemoveAt(j2);
					i2 -= 1;
				}
			}
		}

		return j;
	}

	/// <summary>
	/// Returns the Voronoi cells that border the given cell.
	/// This ignores clipping.
	/// This may give surprising results in degenerate cases that more than 3 cells meet at a point.
	/// </summary>
	public IEnumerable<int> Neighbors(int i)
	{
		if (this.collinearNormal != null)
		{
			var hi = this.hullIndex[i];
			if (hi > 0)
			{
				yield return this.Delaunator.Hull[hi - 1];
			}

			if (hi < this.Delaunator.Hull.Length - 1)
			{
				yield return this.Delaunator.Hull[hi + 1];
			}
		}
		else
		{
			if (this.Inedges.Length <= i || this.Inedges[i] == -1)
			{
				yield break;
			}

			var e0 = this.Inedges[i];
			var e = e0;
			do
			{
				var t = this.Delaunator.Triangles[e];
				yield return t;
				var e1 = NextHalfedge(e);
				if (this.Delaunator.Triangles[e1] != i)
				{
					break;// bad triangulation
				}

				e = this.Delaunator.Halfedges[e1];
				if (e == -1)
				{
					yield return this.Delaunator.Triangles[NextHalfedge(e1)];
					break;
				}
			}
			while (e != e0);
		}
	}

	/// <summary>
	/// Returns the Voronoi cells that border the given cell.
	/// This uses clipping.
	/// </summary>
	public IEnumerable<int> ClippedNeighbors(int i)
	{
		var ci = this.GetClippedPolygon(i);
		if (ci == null)
		{
			yield break;
		}

		foreach (var j in this.Neighbors(i))
		{
			var cj = this.GetClippedPolygon(j);
			if (cj == null)
			{
				continue;
			}

			// find the common edge
			var li = ci.Count;
			var lj = cj.Count;
			for (var ai = 0; ai < li; ai++)
			{
				for (var aj = 0; aj < lj; aj++)
				{
					if (ci[ai] == cj[aj]
					    && ci[(ai + 1) % li] == cj[(aj + lj - 1) % lj]
					   )
					{
						yield return j;
						ai = li;
						break;
					}
				}
			}
		}
	}

	/// <summary>
	/// Finds the voronoi cell that contains the given point, or equivalently,
	/// finds the point that is nearest the given point.
	/// This ignores clipping, so it always succeeds.
	/// </summary>
	/// <param name="u">The point to search for.</param>
	/// <param name="i">Optional, the voronoi cell to start the search at. Useful if you know the returned cell will be nearby.</param>
	/// <returns></returns>
	public int Find(Vector2 u, int i = 0)
	{
		// TODO: Is this a NaN check?
		//if ((x = +x, x !== x) || (y = +y, y !== y)) return -1;
		var i0 = i;
		int c;
		while ((c = this.Step(i, u)) >= 0 && c != i && c != i0)
		{
			i = c;
		}

		return c;
	}

	// Is u inside voronoi cell i?
	private bool Contains(int i, Vector2 u)
	{
		// TODO: Is this a NaN check?
		//if ((x = +x, x != x) || (y = +y, y != y)) return false;
		return this.Step(i, u) == i;
	}

	// Finds a point that is closer to u than points[i],
	// or returns i if there isn't one.
	private int Step(int i, Vector2 u)
	{
		if (this.collinearNormal != null)
		{
			var c = i;
			var dcp = u - this.points[i];
			var dc = (dcp.x * dcp.x) + (dcp.y * dcp.y);

			var hi = this.hullIndex[i];
			if (hi > 0)
			{
				var t = this.Delaunator.Hull[hi - 1];
				var dtp = u - this.points[t];
				var dt = (dtp.x * dtp.x) + (dtp.y * dtp.y);
				if (dt < dc)
				{
					dc = dt;
					c = t;
				}
			}

			if (hi < this.Delaunator.Hull.Length - 1)
			{
				var t = this.Delaunator.Hull[hi + 1];
				var dtp = u - this.points[t];
				var dt = (dtp.x * dtp.x) + (dtp.y * dtp.y);
				if (dt < dc)
				{
					dc = dt;
					c = t;
				}
			}

			return c;
		}
		else
		{
			if (this.Inedges[i] == -1 || this.points.Count == 0)
			{
				return (i + 1) % this.points.Count;
			}

			var c = i;
			var dcp = u - this.points[i];
			var dc = (dcp.x * dcp.x) + (dcp.y * dcp.y);
			var e0 = this.Inedges[i];
			var e = e0;
			do
			{
				var t = this.Delaunator.Triangles[e];
				var dtp = u - this.points[e];
				var dt = (dtp.x * dtp.x) + (dtp.y * dtp.y);
				if (dt < dc)
				{
					dc = dt;
					c = t;
				}

				e = NextHalfedge(e);
				if (this.Delaunator.Triangles[e] != i)
				{
					break;// bad triangulation
				}

				e = this.Delaunator.Halfedges[e];
				if (e == -1)
				{
					e = this.Delaunator.Hull[(this.hullIndex[i] + 1) % this.Delaunator.Hull.Length];
					if (e != t)
					{
						var etp = u - this.points[e];
						var et = (etp.x * etp.x) + (etp.y * etp.y);

						if (et < dc)
						{
							return e;
						}
					}

					break;
				}
			}
			while (e != e0);

			return c;
		}
	}

	// Given a ray of origin u, and direction v,
	// Finds the intersection point with the rectangle bounds.
	// Returns null if the origin starts outside the bounds.
	private Vector2? Project(Vector2 u, Vector2 v)
	{
		var t = float.PositiveInfinity;
		float c;
		var r = new Vector2();
		if (v.y < 0)
		{
			// top
			if (u.y <= this.clipMin.y)
			{
				return null;
			}

			if ((c = (this.clipMin.y - u.y) / v.y) < t)
			{
				r = new Vector2(u.x + ((t = c) * v.x), this.clipMin.y);
			}
		}
		else if (v.y > 0)
		{
			// bottom
			if (u.y >= this.clipMax.y)
			{
				return null;
			}

			if ((c = (this.clipMax.y - u.y) / v.y) < t)
			{
				r = new Vector2(u.x + ((t = c) * v.x), this.clipMax.y);
			}
		}

		if (v.x > 0)
		{
			// right
			if (u.x >= this.clipMax.x)
			{
				return null;
			}

			if ((c = (this.clipMax.x - u.x) / v.x) < t)
			{
				r = new Vector2(this.clipMax.x, u.y + ((t = c) * v.y));
			}
		}
		else if (v.x < 0)
		{
			// left
			if (u.x <= this.clipMin.x)
			{
				return null;
			}

			if ((c = (this.clipMin.x - u.x) / v.x) < t)
			{
				r = new Vector2(this.clipMin.x, u.y + ((t = c) * v.y));
			}
		}

		return r;
	}

	private int EdgeCode(Vector2 v)
	{
		return (v.x == this.clipMin.x
			       ? 0b0001
			       : v.x == this.clipMax.x
				       ? 0b0010
				       : 0b0000)
		       | (v.y == this.clipMin.y
			       ? 0b0100
			       : v.y == this.clipMax.y
				       ? 0b1000
				       : 0b0000);
	}

	private int RegionCode(Vector2 v)
	{
		return (v.x < this.clipMin.x
			       ? 0b0001
			       : v.x > this.clipMax.x
				       ? 0b0010
				       : 0b0000)
		       | (v.y < this.clipMin.y
			       ? 0b0100
			       : v.y > this.clipMax.y
				       ? 0b1000
				       : 0b0000);
	}

	#endregion
}
